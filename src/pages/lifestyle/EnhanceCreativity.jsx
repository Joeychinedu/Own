...
function EnhanceCreativity() {
  ...
  const matchCategory = category?.filter((el) => el.id === 1);

  const matchCategoryItems = posts?.filter((el) => el.CategoryId === 1);

  const relatedCategoryItems = posts?.filter(
    (el) =>
      el.CategoryId === 1 && el.title !== "Do Drugs Really Enhance Creativity"
  );

  const DisplayCategory = matchCategory?.filter(
    (el) => el.name === "Lifestyle"
  );
  const DisplayCategoryItem = matchCategoryItems?.filter(
    (el) => el.title === "Do Drugs Really Enhance Creativity"
  );
...
export default EnhanceCreativity;
